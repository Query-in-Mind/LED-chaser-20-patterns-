...
int lastButtonState = HIGH; // Last button state (HIGH due to pull-up)
...
void loop() {
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) {
    // If button is pressed (LOW), advance to next pattern
    if (buttonState == LOW && lastButtonState == HIGH) {
      currentPattern = (currentPattern + 1) % patternCount; // Move to next pattern
    }
  }
